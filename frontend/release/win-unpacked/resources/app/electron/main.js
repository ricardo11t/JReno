...
ipcMain.handle('process-files', async (event, pdfPath, excelPath) => {
    let baseForBackendScripts; // Variável que armazenará o caminho base para os scripts de backend
    let pathToPopplerBin; // Variável para o caminho dos binários do Poppler
    if (!app.isPackaged) {
